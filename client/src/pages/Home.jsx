...
const testimonials = [
  {
    quote: "My routine finally feels simple, premium, and consistent.",
    name: "Shreya Rimal",
  },
  {
    quote: "The textures are light, elegant, and perfect for daily use.",
    name: "Monika Bhattarai",
  },
  {
...
function SectionHeader({ label, title, light = false }) {
  return (
    <div className="mb-8">
      <p
        className={`font-body text-[11px] font-bold uppercase tracking-[0.22em] ${light ? "text-white/70" : "text-homa-red"}`}
      >
        {label}
      </p>
      <h2
        className={`text-h2 mt-3 font-heading font-semibold ${light ? "text-white" : "text-homa-black"}`}
      >
        {title}
      </h2>
    </div>
    ...
    <main className="min-h-screen bg-homa-cream">
      ...
      <div className="overflow-hidden bg-homa-black py-4 text-white">
        <div className="animate-[marquee_24s_linear_infinite] whitespace-nowrap font-body text-xs font-bold uppercase tracking-[0.15em]">
          Authentic Japanese Products - Delivered Across Nepal - Sugi Pharmacy
          Certified - Skin Health & Beauty - Authentic Japanese Products -
          Delivered Across Nepal - Sugi Pharmacy Certified - Skin Health &
          Beauty - Authentic Japanese Products - Delivered Across Nepal - Sugi
          Pharmacy Certified - Skin Health & Beauty - Authentic Japanese
          Products - Delivered Across Nepal - Sugi Pharmacy Certified - Skin
          Health & Beauty -
        </div>
      </div>
      ...
      <section className="bg-homa-cream px-5 py-16 md:px-12">
        <div className="mx-auto max-w-7xl">
          <SectionHeader
            label="New Arrivals"
            title="Fresh rituals for everyday skin health"
          />
      ...
      <section className="bg-homa-blush px-5 py-16 md:px-12">
        <div className="mx-auto max-w-7xl">
          <SectionHeader
            label="Best Sellers"
            title="Beloved formulas customers return to"
          />
      ...
      <section className="sakura-pattern bg-homa-red px-5 py-16 text-white md:px-12">
        <div className="mx-auto max-w-7xl text-center">
          <SectionHeader
            ...
            title="Japanese beauty, modern transformation"
            light
          />
          <div className="mx-auto flex max-w-4xl flex-wrap justify-center gap-3">
            {attributes.map((attribute) => (
              <span
                key={attribute}
                className="rounded-pill bg-homa-red-dark px-6 py-3 font-body text-sm font-bold uppercase tracking-[0.12em] text-white"
              >
                {attribute}
              </span>
            ))}
      ...
      <section className="bg-homa-black px-5 py-14 md:px-12">
        <div className="mx-auto grid max-w-7xl gap-8 text-center md:grid-cols-3">
          {stats.map((stat) => (
            <div key={stat.label}>
              <p className="font-heading text-[64px] font-semibold leading-none text-homa-red">
                {stat.value}
              </p>
              <p className="mt-3 font-body text-sm font-bold uppercase tracking-[0.14em] text-white/70">
                {stat.label}
              </p>
            </div>
          ))}
      ...
      <section className="bg-homa-cream px-5 py-16 md:px-12">
        <div className="mx-auto max-w-7xl">
          <SectionHeader
            label="Testimonials"
            title="Notes from the HOMA community"
          />
          <div className="grid gap-5 md:grid-cols-3">
            {testimonials.map((item) => (
              ...
              >
                <p className="font-heading text-lg italic leading-8 text-homa-black">
                  "{item.quote}"
                </p>
                <p className="mt-5 font-body text-sm font-bold uppercase tracking-[0.1em] text-homa-red">
                  {item.name}
                </p>
              </article>
            ))}
